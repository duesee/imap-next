...
impl FragmentKind {
    fn from_info(frament_info: FragmentInfo) -> Self {
        match frament_info {
            FragmentInfo::Line { .. } => FragmentKind::Line,
            FragmentInfo::Literal { .. } => FragmentKind::Literal,
        }
    }
}
...
pub fn read_fragment(fragmentizer: &mut Fragmentizer) -> Option<FragmentInfo> {
    ...
    trace!(
        type = ?FragmentKind::from_info(fragment_info),
